...
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from '@fastbase/ui/components/dialog'
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@fastbase/ui/components/form'
...
function SocialAuthForm({ type }: { type: Type }) {
  ...
  useAsyncEffect(async () => {
    if (isManualAuthOpen) {
      ...
      if (text.startsWith('fastbase://session')) {
  ...
  useAsyncEffect(async () => {
    if (manualAuthUrl.startsWith('fastbase://session')) {
